...
const {polygon, booleanWithin, intersect, kinks} = require('@turf/turf')
...
function hasKinks(polygon) {
  return kinks(polygon).features.length > 0
}

function isValid(geometry) {
  if (geometry.type === 'Polygon') {
    return !hasKinks(geometry) && polygonIsValid(geometry.coordinates)
  }
  if (geometry.type === 'MultiPolygon') {
    return !hasKinks(geometry) && multiPolygonIsValid(geometry.coordinates)
  }
}
